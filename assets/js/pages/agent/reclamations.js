...
function getAgentReclamations() {
  fetch("../../api/agent_reclamations_api.php")
    .then((res) => res.json())
    .then((data) => {
      agent_reclamations.length = 0;
      console.log(data);
      agent_reclamations.push(...data.agent_reclamations);
      applyFilters();
    })
    ...
    .map(
      (r, i) => `
        <tr style="animation-delay:${i * 0.03}s" data-id="${r.id}">
          ...
          <td class="table-objet">${r.client}</td>
  ...
  document.getElementById("confirmDelete").onclick = async function () {
    try {
      const res = await fetch(
        `/complaint-manager/actions/reclamations/supprimer.php?id=${id}`,
        {
          method: "POST",
        },
      );
